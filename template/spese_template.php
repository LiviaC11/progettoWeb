<div class="container my-4">
    ...
    <?php foreach($templateParams["spese"] as $spesa): ?>
        <?php 
            // FIX: Recuperiamo il nome di chi ha pagato in modo sicuro
            $nome_pagante = isset($spesa["nome"]) ? $spesa["nome"] : "Utente rimosso";
            
            // Se il nome non c'è in $spesa, proviamo a cercarlo nella lista coinquilini
            if ($nome_pagante == "Utente rimosso" && isset($templateParams["coinquilini"])) {
                foreach ($templateParams["coinquilini"] as $c) {
                    if ($c["id_utente"] == $spesa["chi_ha_pagato"]) {
                        $nome_pagante = $c["nome"];
                        break;
                    }
                }
            ...
            <div class="card-body">
                <div class="d-flex justify-content-between align-items-start mb-3">
                    <div>
                        ...
                        <!-- Qui usiamo la variabile sicura $nome_pagante -->
                        Pagato da: <strong><?php echo htmlspecialchars($nome_pagante); ?></strong>
                        <div class="small text-muted"><?php echo date("d/m/Y", strtotime($spesa["data_spesa"])); ?></div>
                    </div>
                    <div class="text-end">
                ...
                <div class="border-top pt-3 mt-2">
                    ...
                    <div class="row g-2">
                        <?php 
                        // Calcolo sicuro della quota
                        $num_persone = (isset($templateParams["num_persone"]) && $templateParams["num_persone"] > 0) ? $templateParams["num_persone"] : 1;
                        $quota_singola = $spesa["importo"] / $num_persone; 
                        
                        // Qui $coinquilino esiste perché è definito nel foreach
                        if(isset($templateParams["coinquilini"])):
